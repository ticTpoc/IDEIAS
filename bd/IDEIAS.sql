...
*/
select count(l.publicacao) as contagem,p.id, p.likes,p.usuario,p.titulo,p.texto,p.assunto,p.dia,u.avatar,u.email,u.nome,u.sobrenome
      from likes as l join publicacoes as p on p.id=l.publicacao join usuarios as u on l.usuario=u.email group by l.publicacao order by contagem desc;

select p from likes join usuarios on likes.usuario=usuarios.email join publicacoes on publicacoes.id=likes.publicacao where likes.usuario='william_190_@hotmail.com';
